...
$title = trim($object->getTitle());
    ...
    {
        $select = $this->_getReadAdapter()->select();
        $select->from(array('main' => $this->getMainTable()), array('hash_options'));
        $select->join(
            array('rel' => $this->getTable('customoptions/relation')),
            'rel.group_id = main.group_id AND rel.product_id = ' . intval($product_id),
            array()
        );
        $select->group('main.group_id');
