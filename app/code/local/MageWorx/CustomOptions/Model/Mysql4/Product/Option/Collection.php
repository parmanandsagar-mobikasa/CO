...
class MageWorx_CustomOptions_Model_Mysql4_Product_Option_Collection extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Option_Collection
{
    public function getOptions($storeId)
    {
        $this->getSelect()
            ...
            ->joinLeft(array('default_option_block_title' => $this->getTable('customoptions/option_block_title')),
                '`default_option_block_title`.option_id = `main_table`.option_id AND '.$this->getConnection()->quoteInto('`default_option_block_title`.store_id = ?', 0),
                array('default_block_title' => 'block_title'))
            ->joinLeft(array('store_option_block_title' => $this->getTable('customoptions/option_block_title')),
                '`store_option_block_title`.option_id = `main_table`.option_id AND '.$this->getConnection()->quoteInto('`store_option_block_title`.store_id = ?', $storeId),
                array('store_block_title' => 'block_title',
                'block_title' => new Zend_Db_Expr('IFNULL(`store_option_block_title`.block_title,`default_option_block_title`.block_title)')))
    ...
    public function addBlockTitleToResult($storeId)
    {
        $this->getSelect()
            ->joinLeft(array('default_option_block_title' => $this->getTable('customoptions/option_block_title')),
                '`default_option_block_title`.option_id = `main_table`.option_id AND '.$this->getConnection()->quoteInto('`default_option_block_title`.store_id = ?', 0),
