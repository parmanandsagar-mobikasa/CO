...
try {         
        ...
        if ($storeId==0) {
            ...
            if (isset($option['option_code'])) $optionData['option_code'] = $option['option_code'];
        ...
        if (isset($option['block_title'])) {
            $optionBlockTitle = array(
                'option_id' => $optionId,
                'store_id' => $storeId,
                'block_title' => $option['block_title']
            );

            if ($storeId>0) {
                $select = $connection->select()->from($tablePrefix . 'custom_options_option_block_title', array('block_title'))->where('option_id = '.$optionId.' AND `store_id` = '.$storeId);
                $updateFlag = $connection->fetchOne($select);
            }

            if ($option['block_title']!==$updateFlag) {
                if ($updateFlag) {
                    $connection->update($tablePrefix . 'custom_options_option_block_title', $optionBlockTitle, 'option_id = '.$optionId.' AND `store_id` = '.$storeId);
                } else {
                    $connection->insert($tablePrefix . 'custom_options_option_block_title', $optionBlockTitle);
                }
            }
        } elseif ($storeId>0) {
            $connection->delete($tablePrefix . 'custom_options_option_block_title', 'option_id = '.$optionId.' AND store_id = '.$storeId);
        }   
    ...
    public function saveOptionValue($optionId, $value, $optionTypeId = 0, $storeId = 0) {
        ...
        if ($storeId==0) {
            ...
            if (isset($value['code'])) $optionValue['code'] = $value['code'];
            if (isset($value['tier'])) { $tiers = array(); foreach($value['tier'] as $k=>$v) { $tiers[] = $v; } $optionValue['tier_price'] = json_encode($tiers); }
    ...
    private function _removeOptionBlockTitle($id) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $connection->delete($tablePrefix . 'custom_options_option_block_title', 'option_id = ' . $id);
    }
    ...
    private function _removeRelationOptions($relationOptionIds) {
        if (isset($relationOptionIds) && is_array($relationOptionIds)) {
            foreach ($relationOptionIds as $id) {
                ...
                $this->_removeOptionBlockTitle($id);
                ...
                if (isset($value['option_type_id'])) {    
                    ...
                    $optionValue = array(
                       ...
                       'sku' => $value['sku'],
                        ...
                        'tier_price' => isset($value['tier']) ? json_encode($value['tier']) : '{}',
    ...
    public function getProductOptionCollection(Mage_Catalog_Model_Product $product) {                
        ...
        if (Mage::app()->getStore()->isAdmin()) {           
            $collection = $this->getCollection()->addFieldToFilter('product_id', $product->getId())
                ...
                ->addBlockTitleToResult($product->getStoreId())
    ...
    public function getIsRequire() {
        ...
        if (       ($moduleName=='checkout' && $actionName!='configure') 
                ...
                || (($controllerName=='sales_order_edit' || $controllerName=='sales_order_create' || $controllerName=='orderspro_order_edit') && ($actionName=='loadBlock' || $actionName=='index'))
                || $actionName=='reorder'
           ) {            
