...
if(!isset($fileOrig)) return '';
     ...
     */
    protected function _getTierPriceBlock()
    {
        if (!Mage::registry('tier-price-block')) {
            /** @var $block Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Price_Tier */
            $block = Mage::app()->getLayout()->createBlock('adminhtml/catalog_product_edit_tab_price_tier');
            /** @var $attribute Mage_Eav_Model_Entity_Attribute */
            $attribute = Mage::getModel('catalog/resource_eav_attribute')
                ->loadByCode('catalog_product', 'tier_price');
            $block->setElement(new Varien_Object(array('html_id' => 'test', 'entity_attribute' => $attribute)));
            Mage::register('product', Mage::getModel('catalog/product'));
            Mage::register('tier-price-block', $block);
        }
        return Mage::registry('tier-price-block');
    }

    public function getTierPriceTemplate()
    {
        $html = $this->_getTierPriceBlock()->setTemplate('customoptions/tier-price-template.phtml')->toHtml();
        return str_replace(array("\n", "\r", "\0", "\t"), '', $html);
    }

    public function getTierPriceScript()
    {
        return $this->_getTierPriceBlock()->setTemplate('customoptions/tier-price-script.phtml')->toHtml();
    }
     ...
     * @return array
     */
    public function getProductTierPrice(Mage_Catalog_Model_Product $product)
    {
        $productOptions = Mage::getModel('customoptions/group')->getProductOptions($product->getId());
        $tierPrice = array();
        if (is_array($productOptions) && count($productOptions)) {
            foreach ($productOptions as $_options) {
                if (is_array($_options) && count($_options)) {
                    foreach ($_options as $_option) {
                        if (count($_option['values'])) {
                            foreach ($_option['values'] as $_value) {
                                if (isset($_value['tier']) && is_array($_value['tier'])) {
                                    if (count($_value['tier'])) {
                                        $price = array('title' => $_value['title']);
                                        foreach ($_value['tier'] as $_tier) {
                                            $price['values'][$_tier['price_qty']] = $_tier['price'];
                                        }
                                       $tierPrice[$price['title']] = $price;
                                    }
                                }
                            } 
                        }
    ...
    /**
     * get option by option code from decorated option array
     * if $pop = 1 this option will be deleted from array
     *
     * @param array $options
     * @param string $code
     * @param int $pop
     * @return MageWorx_CustomOptions_Model_Catalog_Product_Option
     */
    public function getByOptionCode(&$options, $code, $pop = 0)
    {
        foreach ($options as $_key => $_option) {
            if ($_option->getOptionCode() == $code) {
                if ($pop) {
                    unset($options[$_key]);
                }
                return $_option;
