...
class MageWorx_CustomOptions_Block_Catalog_Product_View_Options_Type_Select extends Mage_Catalog_Block_Product_View_Options_Type_Select {
    ...
    public function getValuesHtml() {
        /** @var $_option MageWorx_CustomOptions_Model_Catalog_Product_Option */
        ...
        $formData = Mage::getSingleton('checkout/session')->getAddToCartFormData();
        $formValue = isset($formData['options'][$_option->getid()]) ? $formData['options'][$_option->getid()] : null;
        ...
        if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE) {            
            ...
            if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN) {
                $select->setName('options[' . $_option->getid() . ']')
                        ->addOption('', $this->__('Kies uw lettertype'));
            } else {
                $select->setName('options[' . $_option->getid() . '][]');
            ...
            foreach ($_option->getValues() as $_value) {                
                ...
                if ($enabledDependent) {
                    ...
                    if ($_value->getInGroupId()) {
                        $dependentJs .= 'inGroupIdData['.$_value->getInGroupId().'] = {"disabled":'.($_option->getIsDependent()?'true':'false').', "select_'.$_option->getId().'":['.$_value->getOptionTypeId().', "'.addslashes($_value->getTitle().' '.$priceStr.$qty).'", '.(($enabledInventory && $_value->getCustomoptionsQty()==='0')?'0':'1').']}; ';
                    }
                } 
                
                $imagePath = $_value->getImagePath();
...
$fileName =  $file;
$selectOptions['image_path'] = Mage::getBaseUrl('media') . 'customoptions/'. $imagePath. '/' . $fileName;
}
}
            ...
            if ($configValue || $formValue) {
                $value = $formValue ? $formValue : $configValue;
                $select->setValue($value);
                if ($enabledDependent) $dependentJs .= 'dependentDefault["select_' . $_option->getId().'"] = '.$value.';';
            }
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                if ($disabled && $helper->hideDependentOption()) $selectHtml .= '<li style="display: none;">'; else $selectHtml .= '<li>';
                ...
                if ($configValue || $formValue) {
                    ...
                    if ($arraySign) {
                        $checked = (is_array($value) && in_array($htmlValue, $value)) ? 'checked' : '';
                    } else {
                        $checked = ($value == $htmlValue ? 'checked' : '');
                    }
                } else {
                ...
                if ($helper->isQntyInputEnabled() && $_option->getQntyInput() && $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_CHECKBOX) {                    
                    ...
                    $validate = $_option->getIsRequire()?"validate-greater-than-zero":"validate-zero-or-greater";
                    $selectHtml .=
                        ...
                        '<span class="label">
                            ...
                            &nbsp;&nbsp;&nbsp;
                        <label><b>'.$helper->getDefaultOptionQtyLabel().'</b> <input type="text" class="input-text qty '.$validate.'" value="'.$optionValueQty.'" maxlength="12" id="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" name="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" onchange="'.((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').'" onKeyPress="if(event.keyCode==13){'.((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').'}" '.($checked?$disabled:'disabled').'></label>
                         </span>';
                } else {

                    $color = ($_value->getCode()) ? '<div style="border:1px solid;border-radius: 5px; width: 12px; height: 12px; display: inline-block; background-color: ' . $_value->getCode() . '"></div> ' : '';
$style='';
                    if($color || $_value->getImagePath())  $style='style="position:absolute; left:-10000px"';
			$selectHtml .= '<input ' . $disabled . ' ' . $checked . ' '.$style.' type="' . $type . '" class="' . $class . ' ' . $require . ' product-custom-option" onclick="'.(($enabledDependent)?'dependentOptions.select(this);':'').((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').$setProdutQtyFunc.'" name="options[' . $_option->getId() . ']' . $arraySign . '" id="options_' . $_option->getId() . '_' . $count . '" value="' . $_value->getOptionTypeId() . '" />' .
                        '<span class="label"';
                        if($color)  $selectHtml.=' style="margin:0 5px 20px 0" ';
                        $selectHtml.='><label for="options_' . $_option->getId() . '_' . $count . '">' . $color . $_value->getTitle() . ' ' . $priceStr . $qty . '</label></span>';
                }
